...
class MockEmbedding:
    ...
    def embed(self, texts):
        ...
        return [[float(len(text)), ord(text[0]) if text else 0.0, 1.0] for text in texts]
...
def sample_documents(temp_dir):
    ...
    doc2_path.write_text(
        "This is the second document about natural language processing and deep learning."
    )
...
class TestFullPipeline:
    ...
    def test_full_pipeline_with_mocks(self, temp_dir, sample_documents):
        ...
        store = ChromaStore(collection="test_collection", embedder=embedder, persist_dir=temp_dir)
        ...
        # Create agent
        agent = RagAgent(store=store, llm=llm, top_k=2, distance_threshold=0.5)
    ...
    def test_pipeline_with_no_relevant_docs(self, temp_dir, sample_documents):
        ...
        store = ChromaStore(collection="test_collection_2", embedder=embedder, persist_dir=temp_dir)

        ingest_file(doc1_path, store, source_name="doc1.txt")

        agent = RagAgent(
            store=store, llm=llm, top_k=2, distance_threshold=0.001  # Very restrictive
        )
    ...
    def test_pipeline_with_llm_not_found_response(self, temp_dir, sample_documents):
        ...
        store = ChromaStore(collection="test_collection_3", embedder=embedder, persist_dir=temp_dir)
    ...
    def test_chunking_and_retrieval(self, temp_dir):
        ...
        long_content = (
            "This is section one about topic A. " * 20 + "This is section two about topic B. " * 20
        )
        ...
        store = ChromaStore(collection="test_chunking", embedder=embedder, persist_dir=temp_dir)

        # Ingest with small chunk size to force chunking
        ingest_file(
            str(long_doc_path), store, source_name="long_doc.txt", max_chars=100, overlap=20
        )

        agent = RagAgent(store=store, llm=llm, top_k=5, distance_threshold=0.5)
    ...
    def test_multiple_sources(self, temp_dir):
        ...
        store = ChromaStore(collection="test_multiple", embedder=embedder, persist_dir=temp_dir)
